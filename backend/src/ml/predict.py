...
def predict_college_stats(excel_file):
  ...
  salary_frequency = data[data['salary_as_fresher']>0]['salary_as_fresher'].value_counts()
  # salary_frequency_dict = salary_frequency.to_dict(orient='dict')
  Final_list['salary_distribution']={str(salary): count for salary, count in salary_frequency.items()}
  ...
  if len(final_list_internships) > 0:
    Final_list['min_no_of_internships_to_get_avg_sal'] = int(np.median(final_list_internships))
  else:
  ...
  min_projects = int(np.median(final_list_projects)) if len(final_list_projects) > 0 else 0
  ...
  average_projects_no_skills = int(np.round(projects_no_skills.mean(skipna=True))) if not projects_no_skills.isnull().all() else 0
  ...
  for branch, group_data in data_sal.groupby('branch'):
      cdf_sal_by_branch[branch]=np.unique(group_data['salary_as_fresher']).astype('int').tolist()
  ...
  print(Final_list)
  Annotation_final_json = json.dumps(Final_list)
  Annotation_final_json
  return(Annotation_final_json)
