    ...
    def ResumeParser():
                ...
                for i in data["education"]:
                    if(i.get('accreditation') is not None and i["accreditation"].get("education") is not None and i.get('organization') is not None):
                        ...
                        if ((i["accreditation"]["educationLevel"] is not None and 'bachelors' in i["accreditation"]["educationLevel"].lower()) or (i["accreditation"]["inputStr"] is not None and ('bachelors' in i["accreditation"]["inputStr"].lower() or 'btech' in i["accreditation"]["inputStr"].lower())) or (i["organization"] is not None and 'engineering' in i["organization"].lower())):
                            ...
                            if compare(['cse', 'computer', 'csbs', 'cst'],branch): details["CSE"] = 1 
                            elif compare(['communication' , 'ece'],branch): details['ECE'] = 1
                            elif compare(['mechanical' , 'mech'],branch): details["MECH"] = 1
        ...
        try:
            for i in data["skills"]:
                name = i["name"].lower()
                if compare(["dsa","data structures","algorithms"],name): details['dsa'] = 1
                if compare(['html' , 'css' , 'javascript','mern'],name): details['web_dev'] = 1
                if compare(['machine learning' , 'data science'],name): details["Machine Learning"] = 1
                if compare(['cloud' , 'aws' , 'azure'],name): details['cloud'] = 1
                if compare(['mobile' , 'flutter' , 'react native' , 'swift' , 'kotlin'],name): details['mobile_dev'] = 1
                if compare(['java','c++','python','golang','javascript','c#','php'],name): details['no_of_programming_languages']+=1
        except: print()
